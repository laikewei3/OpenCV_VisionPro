...
namespace OpenCV_Vision_Pro
{
    ...
    public partial class ColorExtractorTool:IToolBase
    {
        ...
        public  void Run(Mat img, Rectangle region)
        {
            ...
            if (region.IsEmpty)
                image = img.Clone();
            else if(((ColorExtractorToolControl)m_toolControl).m_roi.points == null)
                image = new Mat(img,region);
            else
            {
                Mat mask = Mat.Zeros(img.Rows, img.Cols, img.Depth, img.NumberOfChannels);
                CvInvoke.FillPoly(mask, new VectorOfPoint(((ColorExtractorToolControl)m_toolControl).m_roi.points), new MCvScalar(255, 255, 255));
